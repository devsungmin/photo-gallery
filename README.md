...
- 🖼️ **WebP 이미지 포맷**: 모든 사진을 WebP 포맷으로 관리하여 원본 대비 약 75%의 용량 절감 효과를 제공합니다. `make convert` 명령으로 JPG, RAW, HEIC 등을 WebP로 간편하게 변환할 수 있습니다.
- 📁 **폴더 기반 카테고리**: `photos-src` 디렉토리 내에 생성한 폴더 구조를 기반으로 카테고리가 자동으로 분류됩니다.
...
### 2️⃣ 사진 파일 추가하기
사진이나 RAW/HEIC 파일들을 `photos-src` 디렉토리 안에 넣어주세요. 하위 폴더별로 사진을 모아두면, 그 폴더 이름이 **카테고리로 자동 인식**됩니다.
```text
photos-src/
├── 🌿 nature/
│   └── DSC0001.ARW
└── 🏙️ street/
    └── IMG_1234.HEIC
```
> 💡 원본은 `public/` 밖에 두어 빌드 산출물(`dist/`)에 포함되지 않습니다. 실제 배포에는 `generate`가 만든 `public/optimized`·`public/thumbnails`만 사용됩니다.
